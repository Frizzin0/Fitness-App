...
export default function Allenamento() {
  ...
  // Piano per giorno settimana (0=Lun..6=Dom)
    ...
    return (
      <div className="fade-up">
            ...
            {esercizi.map((es, esIdx) => (
              <div key={esIdx} style={{ marginBottom: 16, background: '#1a1a1e', borderRadius: 10, padding: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <button onClick={() => setEsercizi(esercizi.filter((_, i) => i !== esIdx))}
                    style={{ background: 'none', border: 'none', color: '#ff4757', cursor: 'pointer', fontSize: 16 }}>×</button>
                </div>
                ...
                {es.serie.map((sr, srIdx) => (
                  <div key={srIdx} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: 6, marginBottom: 6, alignItems: 'center' }}>
                    ...
                    <button onClick={() => { const u = [...esercizi]; u[esIdx].serie = u[esIdx].serie.filter((_, i) => i !== srIdx); setEsercizi(u) }}
                      style={{ background: 'none', border: 'none', color: '#4a4a55', cursor: 'pointer', fontSize: 14 }}>×</button>
                  </div>
                ))}
        ...
        <FieldGroup label="Note">
          <textarea placeholder="Sensazioni, progressi..." value={noteSession} onChange={e => setNoteSession(e.target.value)}
  ...
  return (
    <div className="fade-up">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button onClick={() => setStep('new-session')} style={primaryBtnStyle}>+ Nuova</button>
      </div>

      {/* Calendario settimana corrente */}
      <div style={{ background: '#161618', border: '1px solid #2a2a2f', borderRadius: 16, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 6 }}>
          {GIORNI.map((g, idx) => {
            ...
            const pianoDel = pianoByGiorno.get(idx) // piano_allenamento usa 0=Lun..6=Dom
              ...
              <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                ...
                <div style={{
                  ...
                  background: ss.length > 0
                    ? `${TIPO_COLOR[ss[0].tipo] || '#4a4a55'}22`
                  ...
                  fontSize: 14,
                  opacity: isPast && ss.length === 0 ? 0.4 : 1,
                }}>
                  {ss.length > 0
                ...
                <span style={{ fontSize: 8, color: isT ? '#e8ff47' : '#4a4a55', fontFamily: 'var(--font-mono)' }}>
                  {format(d, 'd')}
                </span>
              </div>
            )
        ...
        {/* Legenda piano */}
        {piano.filter(p => p.tipo !== 'riposo').length > 0 && (
          <div style={{ marginTop: 14, borderTop: '1px solid #1a1a1e', paddingTop: 12 }}>
            {piano.filter(p => p.tipo !== 'riposo').map(p => (
              <div key={p.id} style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 5 }}>
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: TIPO_COLOR[p.tipo] || '#6b6b74', minWidth: 28 }}>
      ...
      <div style={{ background: '#161618', border: '1px solid #2a2a2f', borderRadius: 16, padding: '18px 0' }}>
        ...
        {sessioni.length === 0 ? (
          ...
          sessioni.map((s, i) => (
            <div key={s.id} style={{ padding: '12px 16px', borderTop: i > 0 ? '1px solid #1a1a1e' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                <div style={{
                  width: 36, height: 36, borderRadius: 8,
                  background: `${TIPO_COLOR[s.tipo] || '#4a4a55'}18`,
                  border: `1px solid ${TIPO_COLOR[s.tipo] || '#4a4a55'}33`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: '#f0f0f2', margin: '0 0 4px', lineHeight: 1.3 }}>
                      {s.titolo || s.tipo}
                    </p>
...
function Pill({ label, color }: { label: string; color?: string }) {
  return (
    <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: color || '#6b6b74', background: '#1a1a1e', padding: '2px 6px', borderRadius: 4 }}>
      {label}
    </span>
  )
}
...
const inputStyle: React.CSSProperties = {
  ...
  fontSize: 14, fontFamily: 'var(--font-body)', outline: 'none', marginBottom: 0,
}
...
const chipStyle: React.CSSProperties = {
  ...
  padding: '6px 12px', color: '#9898a8', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-body)',
}
const primaryBtnStyle: React.CSSProperties = {
  background: '#e8ff47', color: '#0d0d0f', border: 'none',
  borderRadius: 10, padding: '12px 20px', fontSize: 13,
  fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-body)', letterSpacing: '0.02em',
}
...
const sectionTitle: React.CSSProperties = {
  fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 800, color: '#f0f0f2', margin: 0,
}
