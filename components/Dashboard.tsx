...
export default function Dashboard() {
  ...
  const [pastiSettimana, setPastiSettimana] = useState<PianoPasto[]>([])
  const [dispensaWarn, setDispensaWarn] = useState<Dispensa[]>([])
  const [loading, setLoading] = useState(true)

  // Data locale (browser dell'utente = Roma)
  ...
  useEffect(() => {
    async function load() {
      ...
      // Pasti di oggi dalla settimana
      const todayPasti = (stats.pastiSettimana as PianoPasto[]).filter(p => p.data === oggiStr)
      todayPasti.sort((a, b) =>
        MOMENTO_ORDER.indexOf(a.momento) - MOMENTO_ORDER.indexOf(b.momento)
      )
      setPastiOggi(todayPasti)
      setPastiSettimana(stats.pastiSettimana as PianoPasto[])

      const warn = (stats.dispensa as Dispensa[]).filter(
        d => d.soglia_minima != null && d.quantita_disponibile <= (d.soglia_minima ?? 0)
      )
  ...
  const totOggi = pastiOggi.reduce(
    (acc, p) => ({
    ...
    <div className="fade-up" style={{ paddingBottom: 4 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: '#6b6b74', marginTop: 6 }}>
          Target:{' '}
          <span style={{ color: '#e8ff47', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>
            {profilo?.target_kcal} kcal
          </span>{' '}
          · {profilo?.target_proteine}P / {profilo?.target_carboidrati}C / {profilo?.target_grassi}G
        </p>
      </div>
      ...
      <div
        className="fade-up-1"
        style={{ background: '#161618', border: '1px solid #2a2a2f', borderRadius: 16, padding: '18px 16px', marginBottom: 16 }}
      >
      ...
      {pastiOggi.length > 0 && (
        <div
          className="fade-up-2"
          style={{ background: '#161618', border: '1px solid #2a2a2f', borderRadius: 16, padding: '18px 0', marginBottom: 16 }}
        >
          ...
          {pastiOggi.map((p, i) => (
            <div
              key={p.id}
              style={{ display: 'flex', alignItems: 'center', padding: '10px 16px', borderTop: i > 0 ? '1px solid #1a1a1e' : 'none', opacity: p.completato ? 0.5 : 1 }}
            >
              <div
                style={{
              ...
              {p.kcal && (
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#6b6b74' }}>{p.kcal} kcal</span>
              )}
            </div>
          ))}
        </div>
      ...
      >
        ...
        {sessioni.length === 0 ? (
          ...
          sessioni.map((s, i) => (
            <div
              key={s.id}
              style={{ display: 'flex', alignItems: 'center', padding: '10px 16px', borderTop: i > 0 ? '1px solid #1a1a1e' : 'none', gap: 12 }}
            >
              <div
                style={{
      ...
      {/* Grafici storici */}
      <div style={{ background: '#161618', border: '1px solid #2a2a2f', borderRadius: 16, padding: '18px 16px', marginBottom: 16 }}>
        <WorkoutFrequencyChart sessioni={sessioni} />
        <MacroSettimanaChart pasti={pastiSettimana} />
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', gap: 4, overflowX: 'auto' }}>
      {days.map((d, i) => {
        ...
        // date-fns getDay: 0=Dom,1=Lun,...,6=Sab → mappa a L=0..D=6
          ...
          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <div
              style={{
                ...
                background: hasSession ? `${TIPO_COLOR[tipo!] || '#4a4a55'}33` : '#1a1a1e',
                border: isT
                  ? '1px solid #e8ff47'
                  : hasSession
                  ? `1px solid ${TIPO_COLOR[tipo!] || '#4a4a55'}44`
